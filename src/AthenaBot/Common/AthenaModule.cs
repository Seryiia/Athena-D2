...
using MessageType = AthenaBot.Extensions.MessageType;

// ReSharper disable InconsistentNaming

namespace AthenaBot.Modules;

[UsedImplicitly(ImplicitUseTargetFlags.Default
                | ImplicitUseTargetFlags.WithInheritors
                | ImplicitUseTargetFlags.WithMembers)]
public abstract class AthenaModule : ModuleBase
{
    ...
    public IAthenaInteractionService _inter { get; set; }
    ...
    // 
    public Task<IUserMessage> SendErrorAsync(string text, AthenaInteraction inter = null)
        => ctx.Channel.SendAsync(_eb, text, MessageType.Error, inter);
    public Task<IUserMessage> SendConfirmAsync(string text, AthenaInteraction inter = null)
        => ctx.Channel.SendAsync(_eb, text, MessageType.Ok, inter);
    public Task<IUserMessage> SendPendingAsync(string text, AthenaInteraction inter = null)
        => ctx.Channel.SendAsync(_eb, text, MessageType.Pending, inter);

    
    // localized normal
    public Task<IUserMessage> ErrorLocalizedAsync(LocStr str, AthenaInteraction inter = null)
        => SendErrorAsync(GetText(str), inter);

    public Task<IUserMessage> PendingLocalizedAsync(LocStr str, AthenaInteraction inter = null)
        => SendPendingAsync(GetText(str), inter);

    public Task<IUserMessage> ConfirmLocalizedAsync(LocStr str, AthenaInteraction inter = null)
        => SendConfirmAsync(GetText(str), inter);

    // localized replies
    public Task<IUserMessage> ReplyErrorLocalizedAsync(LocStr str, AthenaInteraction inter = null)
        => SendErrorAsync($"{Format.Bold(ctx.User.ToString())} {GetText(str)}", inter);

    public Task<IUserMessage> ReplyPendingLocalizedAsync(LocStr str, AthenaInteraction inter = null)
        => SendPendingAsync($"{Format.Bold(ctx.User.ToString())} {GetText(str)}", inter);

    public Task<IUserMessage> ReplyConfirmLocalizedAsync(LocStr str, AthenaInteraction inter = null)
...
public abstract class AthenaModule<TService> : AthenaModule
{
