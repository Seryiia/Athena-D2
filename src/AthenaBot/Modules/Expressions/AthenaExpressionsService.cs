...
public sealed class AthenaExpressionsService : IExecOnMessage, IReadyExecutor
{
...
";
    ...
    private readonly TypedKey<AthenaExpression> _gexprAddedKey = new("gexpr.added");
    private readonly TypedKey<int> _gexprDeletedkey = new("gexpr.deleted");
    private readonly TypedKey<AthenaExpression> _gexprEditedKey = new("gexpr.edited");
    ...
    private AthenaExpression[] globalExpressions = Array.Empty<AthenaExpression>();
    private ConcurrentDictionary<ulong, AthenaExpression[]> newguildExpressions = new();
    ...
    public AthenaExpressionsService(
        DbService db,
        IBotStrings strings,
        Bot bot,
    ...
    private AthenaExpression TryGetExpression(IUserMessage umsg)
    {
        if (!ready)
            return null;
    ...
    private AthenaExpression MatchExpressions(in ReadOnlySpan<char> content, AthenaExpression[] exprs)
    {
        var result = new List<AthenaExpression>(1);
    ...
    private Task UpdateInternalAsync(ulong? maybeGuildId, AthenaExpression expr)
    {
    ...
    private void UpdateInternal(ulong? maybeGuildId, AthenaExpression expr)
    {
        if (maybeGuildId is { } guildId)
        {
    ...
    private Task AddInternalAsync(ulong? maybeGuildId, AthenaExpression expr)
    {
    ...
    private AthenaExpression[] DeleteInternal(
        IReadOnlyList<AthenaExpression> exprs,
        int id,
        out AthenaExpression deleted)
    {
        deleted = null;
        if (exprs is null || exprs.Count == 0)
            return exprs as AthenaExpression[] ?? exprs?.ToArray();

        var newExprs = new AthenaExpression[exprs.Count - 1];
    ...
    public AthenaExpression GetExpression(ulong? guildId, int id)
    {
    ...
    public async Task<bool> ImportExpressionsAsync(ulong? guildId, string input)
    {
        ...
        foreach (var entry in data)
        {
            ...
            await uow.Expressions.AddRangeAsync(entry.Value.Where(expr => !string.IsNullOrWhiteSpace(expr.Res))
                .Select(expr => new AthenaExpression
                {
                    GuildId = guildId,
                    Response = expr.Res,
    ...
    private ValueTask OnGexprAdded(AthenaExpression c)
    {
        lock (_gexprWriteLock)
        {
            var newGlobalReactions = new AthenaExpression[globalExpressions.Length + 1];
    ...
    private ValueTask OnGexprEdited(AthenaExpression c)
    {
        lock (_gexprWriteLock)
        {
    ...
    public async Task<AthenaExpression> AddAsync(ulong? guildId, string key, string message)
    {
        key = key.ToLowerInvariant();
        var expr = new AthenaExpression
        {
            GuildId = guildId,
            Trigger = key,
            Response = message
    ...
    public async Task<AthenaExpression> EditAsync(ulong? guildId, int id, string message)
    {
        await using var uow = _db.GetDbContext();
        var expr = uow.Expressions.GetById(id);
    ...
    public async Task<AthenaExpression> DeleteAsync(ulong? guildId, int id)
    {
        await using var uow = _db.GetDbContext();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public AthenaExpression[] GetExpressionsFor(ulong? maybeGuildId)
    {
        if (maybeGuildId is { } guildId)
            return newguildExpressions.TryGetValue(guildId, out var exprs) ? exprs : Array.Empty<AthenaExpression>();
